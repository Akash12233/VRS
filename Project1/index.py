...
class ReturnVehicle:
    def __init__(self, root, main_row):
        ...
        def Receiptt():
            ...
            Total_bill = float(str(self.s5.cell(self.main_row,6).value)[6:])*(2000+500+Total_Reading*10) + float(Damage.get())
            ...
            self.txtReceipt2.insert(END, str(self.s5.cell(self.main_row,7).value)+"\n")
            ...
            for a in range (2,12):
                if self.s1.cell(a,2).value == self.s5.cell(self.main_row,5).value:
                    self.s1.cell(a,3).value=float(self.s1.cell(a,3).value)+float(self.s5.cell(self.main_row,7).value)
                    wb.save("Vehicle.xlsx")
                if self.s2.cell(a,2).value == self.s5.cell(self.main_row,5).value:
                    maxrows = self.s2.max_row
                    self.s2.cell(a,3).value=float(self.s2.cell(a,3).value)-float(self.s5.cell(self.main_row,7).value)
                    wb.save("Vehicle.xlsx")
                if self.s3.cell(a,2).value == self.s5.cell(self.main_row,5).value:
                    maxrows = self.s3.max_row
                    self.s3.cell(a,3).value=float(self.s3.cell(a,3).value)-float(self.s5.cell(self.main_row,7).value)
                    wb.save("Vehicle.xlsx")
                if self.s4.cell(a,2).value == self.s5.cell(self.main_row,5).value:
                    maxrows = self.s1.max_row
                    self.s4.cell(a,3).value=float(self.s4.cell(a,3).value)-float(self.s5.cell(self.main_row,7).value)
                    wb.save("Vehicle.xlsx")
        def Back():
            MainFrame.destroy()
            Tops.destroy()
    ...
    #===============================================recipt======================================================================
        ...
        ButtonFrame=LabelFrame(Receipt_BottonFrame, width=450,height=100, relief=RIDGE)
    ...
    def __init__(self,root, Vehicle):
        ...
        def Receiptt():
            ...
            if int(num.get())>int(self.serialNum[6:]):
                Exit= ms.askyesno("Prompt",f"Only {self.serialNum[6:]} available here")
                if Exit > 0:
                    # root.destroy()
                    Back()
                    return
                ...
                s5.cell(maxrows+1, 7).value = num.get()
                wb.save("Vehicle.xlsx")
